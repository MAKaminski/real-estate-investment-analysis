#!/usr/bin/env python3
"""
Create Google Sheets compatible files with formulas
"""

import csv
import os

def create_google_sheet_template(filename, properties_data, is_post_optimization=False):
    """Create a CSV file that can be imported into Google Sheets"""
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        for client, properties in properties_data.items():
            ...
            for i, prop in enumerate(properties[:3], 1):
                ...
                writer.writerow(['Down Payment (Do not alter)', '20%', '=B' + str(writer.line_num-1) + '*0.2', ''])
                writer.writerow(['Loan Amount', '', '=B' + str(writer.line_num-2) + '-C' + str(writer.line_num-1), ''])
                writer.writerow(['Interest Rate', '6.5%', '', ''])
                writer.writerow(['Loan Term', '30 years', '', ''])
                writer.writerow(['Monthly Payment', '', '=PMT(0.065/12,360,C' + str(writer.line_num-5) + ')', ''])
                writer.writerow(['Closing Costs (3%)', '', '=B' + str(writer.line_num-7) + '*0.03', ''])
                writer.writerow(['Total OOP', '', '=C' + str(writer.line_num-7) + '+C' + str(writer.line_num-1), ''])
                writer.writerow([])
                ...
                writer.writerow(['Operating Expenses', prop['monthly_expenses'], '=B' + str(writer.line_num-1) + '*12', ''])
                writer.writerow(['Net Operating Income', '=B' + str(writer.line_num-7) + '-B' + str(writer.line_num-1), '=C' + str(writer.line_num-1) + '*12', ''])
                writer.writerow(['Cash Flow', '=B' + str(writer.line_num-1) + '-B' + str(writer.line_num-13), '=C' + str(writer.line_num-1) + '*12', ''])
                writer.writerow(['CoC Return', '=C' + str(writer.line_num-1) + '/C' + str(writer.line_num-15), '', ''])
                writer.writerow([])
                
                # Return Analysis
                writer.writerow(['Return Analysis', 'Initial', 'Optimized', 'Improvement'])
                writer.writerow(['Cash on Cash Return', prop['coc_initial'], prop['coc_return'], '=C' + str(writer.line_num-1) + '-B' + str(writer.line_num-1)])
                writer.writerow(['Appreciation (5Y)', prop['appreciation_initial'], prop['appreciation_optimized'], '=C' + str(writer.line_num-1) + '-B' + str(writer.line_num-1)])
                writer.writerow(['Tax Savings', prop['tax_savings_initial'], prop['tax_savings_optimized'], '=C' + str(writer.line_num-1) + '-B' + str(writer.line_num-1)])
                writer.writerow(['Principal Paydown', prop['principal_initial'], prop['principal_optimized'], '=C' + str(writer.line_num-1) + '-B' + str(writer.line_num-1)])
                writer.writerow(['Total Return (5Y)', prop['total_return_initial'], prop['total_return_optimized'], '=C' + str(writer.line_num-1) + '-B' + str(writer.line_num-1)])
                writer.writerow([])
